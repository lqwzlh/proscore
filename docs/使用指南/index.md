...
| 参数集中配置 | [**pipeline-spec.md**](pipeline-spec.md) | **PipelineSpec** — `apply(spec)` 注入各阶段默认参数 | 会写 Python |
...
| **规则挖掘** | [**rules.md**](rules.md) | **`RuleMiner`** — 决策规则挖掘（Lift/Recall/Precision 评估） |
...
| 参数集中配置 | [**pipeline-spec.md**](pipeline-spec.md) | **PipelineSpec** — 链式 API 默认参数（`apply(spec)`） |

## 推荐工作流

```
prefilter（粗筛）→ binning → refine（精筛）→ [mine_rules 可选] → transform → select → fit → scorecard → evaluate
```
