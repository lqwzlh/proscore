...
- [模板详解](#模板详解)
  ...
  - [Sheet 7: Variables — 逐变量预设（可选）](#sheet-7-variables--逐变量预设可选)
- [参数校验规则](#参数校验规则)
- [输出等效代码](#输出等效代码)
- [常见问题](#常见问题)
...
运行后在当前目录生成 `scorecard_report/report.md`（含图表）。
...
模板包含 7 个 Sheet，所有 Sheet 均采用统一列布局：
...
>
> **依赖校验**：refine=on 但 prefilter=off → 报错。select=on 但 refine=off → 报错。
...
| 依赖校验 | refine=on 但 prefilter=off → 报错，refine 依赖 prefilter |
